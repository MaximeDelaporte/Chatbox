...
func (b *Bchatbox) doConnectWebhookBind() error {
	switch {
...
func (b *Bchatbox) doConnectWebhookURL() error {
...
func (b *Bchatbox) apiLogin() error {
...
func (b *Bchatbox) getChannelName(id string) string {
	b.RLock()
	defer b.RUnlock()
...
func (b *Bchatbox) getChannelID(name string) string {
	b.RLock()
	defer b.RUnlock()
...
func (b *Bchatbox) skipMessage(message *models.Message) bool {
	return message.User.ID == b.user.ID
}

func (b *Bchatbox) uploadFile(fi *config.FileInfo, channel string) error {
	fb := gomf.New()
...
func (b *Bchatbox) sendWebhook(msg *config.Message) error {
	// skip events
	if msg.Event != "" {
		return nil
	}
