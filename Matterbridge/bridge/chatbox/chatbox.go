...
type Bchatbox struct {
	c     *chatbox.Chatbox
	User  *chatbox.User
	Users []chatbox.User
	Projects []chatbox.Project
	Rooms []chatbox.Room
	*bridge.Config
}

func New(cfg *bridge.Config) bridge.Bridger {
	return &Bchatbox{Config: cfg}
}

func (b *Bchatbox) Connect() error {
	var err error
	b.Log.Info("Connecting")
	b.c = chatbox.New(b.GetString("Token"))
...
func (b *Bchatbox) Disconnect() error {
	return nil

}

func (b *Bchatbox) JoinChannel(channel config.ChannelInfo) error {
	roomID, err := b.c.GetRoomId(channel.Name)
	if err != nil {
		return fmt.Errorf("Could not find roomID for %v. Please create the room on Chatbox", channel.Name)
	}
	...
	go func(stream *chatbox.Stream, room string) {
		for event := range stream.Event {
			switch ev := event.Data.(type) {
			case *chatbox.MessageReceived:
			...
			case *chatbox.ChatboxConnectionClosed:
				b.Log.Errorf("connection with chatbox closed for room %s", room)
			}
		}
	}(stream, room.URI)
	return nil
...
func (b *Bchatbox) Send(msg config.Message) (string, error) {
...
func (b *Bchatbox) getRoomID(channel string) string {
...
func (b *Bchatbox) getAvatar(user string) string {
	var avatar string
	if b.Users != nil {
...
func (b *Bchatbox) handleUploadFile(msg *config.Message, roomID string) (string, error) {
